...
void begin_bi_ratm(char* settingfile, char* inputfile, char* model_root, char* beta_file = NULL) {
    ...
    Model* model = new Model(num_docs, test_num_docs, train_num_docs,
		num_all_words_in_test, num_all_words_in_train, num_words, num_topics, win);
...
int main(int argc, char* argv[]) {
	...
	if (argc > 1 && argc == 5 && strcmp(argv[1],"est") == 0) {
		...
		printf(
				"./bi-ratm est <setting.txt>  <input data file><model save dir>\n\n");
		printf(
				"If you want to initialize the model with default parameters, please use: \n");
		printf(
				"./bi-ratm est <setting.txt> <input data file> <model save dir> <topic_dis_overwords_beta file >\n\n");
		printf("**************Inference**********************\n");
		printf(
				"./bi-ratm inf <setting.txt> <input test data file> <model dir> <perfix> <output dir>\n");
		printf("\n");
	}
	return 0;
}